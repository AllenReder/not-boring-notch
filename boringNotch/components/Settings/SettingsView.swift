        ...
        VStack {
            Form {
                ...
                HStack(spacing: 30) {
                    Spacer(minLength: 0)
                    Button {
                        if let url = URL(string: "https://github.com/AllenReder/not-boring-notch") {
                            NSWorkspace.shared.open(url)
                        }
                    } label: {
                        VStack(spacing: 5) {
            ...
            VStack(spacing: 0) {
                Divider()
                Text("Not Boring Notch · Maintained with 🫶🏻 by Allen Yi & community")
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
...
struct Appearance: View {
    ...
    @State private var showAdvancedGlassSettings: Bool = true
        ...
        Form {
            ...
            Section {
                ...
                Defaults.Toggle(key: .preventAutoClose) {
                    Text("Pin Notch Open (Keep notch open)")
                }
