...
@main
struct NotBoringNotchApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) var appDelegate
    @Default(.menubarIcon) var showMenuBarIcon
    @Environment(\.openWindow) var openWindow

    var body: some Scene {
        MenuBarExtra("Not Boring Notch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
            ...
            CheckForUpdatesView()
            Divider()
            Button("Restart Not Boring Notch") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? NotBoringNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? NotBoringNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createNotBoringNotchWindow(for screen: NSScreen, with viewModel: NotchViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = NotBoringNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createNotBoringNotchWindow(
        ...
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    ...
                    let window = createNotBoringNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createNotBoringNotchWindow(for: selectedScreen, with: vm)
            }
