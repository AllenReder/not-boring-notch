...
class WebcamManager: NSObject, ObservableObject {
    ...
    private let sessionQueue = DispatchQueue(label: "NotBoringNotch.WebcamManager.SessionQueue", qos: .userInitiated)
        ...
        sessionQueue.async { [weak self] in
            ...
            do {
                ...
                guard session.canAddInput(videoInput) else {
                    throw NSError(domain: "NotBoringNotch.WebcamManager", code: -1, userInfo: [NSLocalizedDescriptionKey: "Cannot add video input"])
                }
